...
#### Aim mode 8
Aim mode 8, also called the meek mode, offers complete randomization. Bullet angle randomization works in the same way as in mode 6, but apart from that, bullets in different layers also ahve their speeds set to random values between `spd1` and `spd1 + spd2`.
[flex]
[code][ins=607,17](0, 8);   
[ins=606,17](0, 10, 3);
