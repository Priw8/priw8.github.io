...
[/flex]
The thing that's really noticable is that there are no "layers" here - every bullet in what would normally be a layer, has a random speed between `spd2` and `spd1`. Other than that, it's a pretty standard statically aimed ring, but things change drastically when we set `ang2` to a non-zero value: 
[flex]
[code][ins=607,17](0, 7);   
[ins=606,17](0, 10, 3);
...
[/flex]
Now our ring has become a garbled mess! Apart from the speed randomization, now the bullets are also offset from their normal angle by a value randomly choosen from the range `[-ang2, ang2]`. Setting `ang2` to a high enough value would completely remove any traces of this even being a ring in the first place.  
  
\.\.\.but what does `ang1` do, then? Well, if you look at the first screenshot of this mode you can notice that the ring is aimed straight right with `ang1` equal 0. Setting `ang1` to `rad(90)` would result in the ring being aimed straight down, etc.. So it basically works the same as in mode 3 in this regard. 

#### Aim mode 8
Aim mode 8, also called the meek mode, offers complete randomization. Bullet angle randomization works in the same way as in mode 6, but apart from that, bullets in different layers also ahve their speeds set to random values between `spd2` and `spd1`. 
[flex]
[code][ins=607,17](0, 8);   
[ins=606,17](0, 10, 3);
...
[/flex]
As you can see, `ang1` had to be set to `rad(90)` to aim the pattern down. There is not much that can be said about this messy bullet cluster - they all have speeds between `1f` and `2f`, and angles between `rad(120)` and `rad(60)`.
